...
import { getHeader, getListData } from 'services/routes/list/standard-list';
...
function* approved(action) {
  try {
    yield put({ type: REPLACE_ROUTES_STATE, payload: { approvalNo: action.payload.approvalNo, goBackRoute: '/list/standard-list' } });
...
function* StandardTable() {
  yield [
    takeLatest(LIST_STANDARD_INIT, action => cancelable(init, action)),
    takeLatest(LIST_STANDARD_GET, action => cancelable(get, action)),
    takeLatest(LIST_STANDARD_APPROVED, action => cancelable(approved, action)),
  ];
}
