...
import { getCity, addTwoColumns } from 'services/routes/form/column-form/two-columns';
...
function* add(action) {
  try {
    ...
    const response = yield call(addTwoColumns, action.payload.formData);
...
function* TwoColumns() {
  yield [
    takeLatest(FORM_TWO_COLUMNS_INIT, action => cancelable(init, action)),
    takeLatest(FORM_TWO_COLUMNS_ADD, action => cancelable(add, action)),
  ];
}
